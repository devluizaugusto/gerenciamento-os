...
const ServiceOrderCard: React.FC<ServiceOrderCardProps> = memo(({ ordem, onEdit, onDelete }) => {
  ...
  return (
    ...
    >
      ...
      {/* Ações — respeitam permissões de role */}
      <footer className="px-4 pb-4 flex gap-2 border-t border-slate-100 pt-3 mt-auto bg-slate-50/80">
        {canEdit ? (
          ...
          <div className="flex-1 flex items-center justify-center gap-1.5 text-xs text-slate-400 bg-slate-100 rounded-lg py-2 cursor-not-allowed select-none">
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
