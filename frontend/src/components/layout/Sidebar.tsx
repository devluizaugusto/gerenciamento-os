  ...
  return (
    <>
      ...
      <aside
        className={`
          hidden md:flex
          relative flex-col h-screen sticky top-0 z-40
          bg-[#1a2236]
      ...
      ══════════════════════════════════ */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#1a2236] border-t border-white/10 safe-area-bottom">
        <div className="flex items-stretch h-16">
          ...
          <button
            ...
            className={`flex-1 flex flex-col items-center justify-center gap-1 transition-all duration-200 relative
              ${currentPage === 'helpdesk' ? 'text-blue-400' : 'text-white/40 active:text-white/70'}`}
          >
            {currentPage === 'helpdesk' && (
              <span className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 bg-blue-400 rounded-b-full" />
            )}
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={currentPage === 'helpdesk' ? 2.2 : 1.8}
          ...
          {currentPage === 'helpdesk' && (
            ...
            >
              <div className="w-10 h-10 rounded-full bg-blue-600 flex items-center justify-center shadow-lg shadow-blue-900/50 -mt-4">
                <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 4v16m8-8H4" />
                </svg>
              </div>
              <span className="text-[10px] font-semibold leading-none">Nova OS</span>
            </button>
          )}

          {/* Action: Gerar PDF (only on helpdesk) */}
          {currentPage === 'helpdesk' && (
            <button
              onClick={onGeneratePDF}
              disabled={!canGeneratePDF}
              className={`flex-1 flex flex-col items-center justify-center gap-1 transition-all duration-200
                ${canGeneratePDF ? 'text-white/40 active:text-white/70' : 'text-white/20'}`}
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8}
                  d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
              <span className="text-[10px] font-semibold leading-none">PDF</span>
            </button>
          )}
          ...
          <button
            ...
            className={`flex-1 flex flex-col items-center justify-center gap-1 transition-all duration-200 relative
              ${currentPage === 'tintas' ? 'text-blue-400' : 'text-white/40 active:text-white/70'}`}
          >
            {currentPage === 'tintas' && (
              <span className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 bg-blue-400 rounded-b-full" />
            )}
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={currentPage === 'tintas' ? 2.2 : 1.8}
                d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
            </svg>
